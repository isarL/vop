...
int EncodePointCloudToFile(const draco::PointCloud &pc, const std::string &file,
                           draco::Encoder *encoder, const bool p) {
  ...
  output_timer.Stop();
  printf("Encoded mesh saved to %s  (%" PRId64 " ms to load, %" PRId64
         " ms to encode and % " PRId64 " ms to save to file).\n",
         file.c_str(), input_timer.GetInMs(), timer.GetInMs(),
         output_timer.GetInMs());
  if (p) {
    ofstream myfile("C:/Users/isarl/Documents/VOP/output.csv",
                    ios::out | ios::app);
    myfile << "," << input_timer.GetInMs() << "," << timer.GetInMs() << ","
            << output_timer.GetInMs() ;
    myfile.close();
  }
...
int EncodeMeshToFile(const draco::Mesh &mesh, const std::string &file,
  ...
  printf("Encoded mesh saved to %s  (%" PRId64 " ms to load, %" PRId64
         " ms to encode and % " PRId64 " ms to save to file).\n",
         file.c_str(), input_timer.GetInMs(),
         timer.GetInMs(), output_timer.GetInMs());
  //printf("\nEncoded size = %zu bytes\n\n", buffer.size());
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  if (input_is_mesh)
    ...
    ret = EncodePointCloudToFile(*pc.get(), options.output, &encoder, options.p);
  /*
